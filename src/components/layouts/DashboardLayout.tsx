...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Mobile sidebar */}
      <div className={`fixed inset-0 z-50 bg-slate-900/80 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`} onClick={() => setSidebarOpen(false)} />
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-white lg:hidden transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
        <div className="flex h-16 items-center justify-between px-6 border-b border-slate-200">
          <Logo />
          <button onClick={() => setSidebarOpen(false)} className="rounded-md p-1 hover:bg-slate-100">
